...
use cbor::value::{canonical_into_writer, canonical_value, CanonicalValue, Value};
use rand::prelude::*;
...
fn map() {
    ...
    cbor::ser::into_writer(&map, &mut bytes1).unwrap();
...
fn tagged_option() {
    ...
    cbor::ser::into_writer(&opt, &mut bytes).unwrap();

    let output = cbor::de::from_reader(&bytes[..]).unwrap();
...
fn canonical_value_example() {
    ...
    cbor::ser::into_writer(&canonical, &mut bytes).unwrap();
